...
def collate(batch):
    material_id, matrix, cart_coords, atomic_numbers, num_atoms, neigh_coords, neigh_types = zip(*batch)
    
    matrix = torch.stack(matrix, 0)
    cart_coords = torch.cat(cart_coords, 0)
    atomic_numbers = torch.cat(atomic_numbers, 0)
    num_atoms = torch.LongTensor(list(num_atoms))
    material_id =list(material_id)

    neigh_coords = torch.cat(neigh_coords, 0)
    neigh_coords = torch.cat([cart_coords.unsqueeze(1), neigh_coords], 1)
    neigh_types = torch.cat(neigh_types, 0)
    neigh_types = torch.cat([atomic_numbers.unsqueeze(1), neigh_types], 1)
    return material_id, matrix.float(), cart_coords.float(), atomic_numbers.long(), num_atoms, neigh_coords.float(), neigh_types.long()
...
def train(rank, world_size, args, config):
    ...
    val = CrystalDataset(args.task, "mp_20", config=config, val=True)
...
if __name__ == '__main__':
    args, config = parse_args_and_config('vae')
